...
//  if (BlynkHeaterOn==1)  { // Disabled Using blynk for on/off (use for feather Huzzah32).
//  if (digitalRead(push_pin)==HIGH) { // Disabled Using digital read of push_pin for on/off(use for feather M0 Express).

  if (analogRead(push_pin)>250) { // Using analogRead of push_pin for on/off (use for feather Huzzah32).
